...
ARGUMENTS = [
    ...
    DeclareLaunchArgument('namespace', default_value='agent_1',
                          description='Robot namespace')
]


def generate_launch_description():
    pkg_multirobot_slam = get_package_share_directory(
        'multirobot_slam')
    ...
    localization_params_arg = DeclareLaunchArgument(
        ...
        default_value=PathJoinSubstitution(
            [pkg_multirobot_slam, 'config', 'localization.yaml']),
        description='Localization parameters')

    map_arg = DeclareLaunchArgument(
        'map',
        default_value=PathJoinSubstitution(
            [pkg_multirobot_slam, 'maps', 'simple.yaml']),
